...
import re
import os
# to extract images from base64 strings (as they are stored in xml files)
import base64
...
df_pause = df_raw.loc[df_raw['style'].str.contains(
    'fillColor=#cdeb8b', na=False), ['id', 'name', 'odk_type']]
...
df.loc[df['required'] == 'yes', 'required'] = '1'
...
def remove_html(string):
    ...
    text = '\n'.join([i.strip(' ') for i in text if i])
    text = text.replace('\n', ' ')
...
df.loc[m, 'value'] = df.loc[m, 'value'].apply(
    lambda x: remove_html(x) if x != None else None)
...
df.loc[m, 'value'] = df.loc[m, 'value'].apply(
    lambda x: html2plain(x) if x != None else None)
...
df.loc[df.duplicated(subset=['name'], keep='first') & ~df['odk_type'].isin(['calculate', 'diagnosis', 'select_option', 'rhombus',
                                                                            'goto', 'container_hint_media', 'help-message', 'hint-message']) & (df['edge'] != '1'), 'name'] = df['name']+df['id']
...
# extract shortcut elements and put in a new dataframe
dfa = df_raw.loc[df_raw['odk_type'] == 'goto'].copy()
dfa.loc[dfa['odk_type'] == 'goto', 'name'] = dfa.loc[dfa['odk_type']
                                                     == 'goto', 'name'].str[9:]  # remove prefix
# merge with raw-data to get the id of the exit element
dfa = dfa.reset_index().merge(df_raw.reset_index()[
    ['id', 'name']], how='left', on='name')
exitmap = dict(zip(dfa['id_x'], dfa['id_y']))  # convert into a dictionnary
# replace the shortcut elements by the exit-element in df_arrows
df_arrows['target'] = df_arrows['target'].replace(exitmap)
df.drop(df.loc[df['odk_type'] == 'goto'].index,
        inplace=True)  # drop shortcuts from df_survey


# %% Make media folder

# recursively create mediafolder, do nothing if it exists
os.makedirs(p.media_folder, exist_ok=True)
...
df.loc[df['style'].str.contains(
    "image/png", na=False), 'odk_type'] = 'png-image'+df.name+'.png'
...
df_pngAll = df.loc[df['odk_type'].str.contains(
    'png-image', na=False), ['xml-parent', 'odk_type', 'style']]
for index, row in df_pngAll.iterrows():
    string = row['style']
    # extract image data from 'style' column using regex
    img_data = re.search('image/png,(.+?);', string).group(1)
    with open(p.media_folder+row['odk_type'], "wb") as fh:
        # encode image into ascii (binary) and save
        fh.write(base64.decodebytes(img_data.encode('ascii')))

df_png.rename({'xml-parent': 'container_id',
              'odk_type': 'image::en'}, axis=1, inplace=True)
...
df.loc[df['style'].str.contains(
    "image/jpeg", na=False), 'odk_type'] = 'jpeg-image'+df.name+'.jpeg'
...
# getting image data from 'style' column for all images (from containers AND select_options) and storing it to disk
df_pngAll = df.loc[df['odk_type'].str.contains(
    'jpeg-image', na=False), ['xml-parent', 'odk_type', 'style']]
for index, row in df_pngAll.iterrows():
    string = row['style']
    # extract image data from 'style' column using regex
    img_data = re.search('image/jpeg,(.+?);', string).group(1)
    with open(p.media_folder+row['odk_type'], "wb") as fh:
        # encode image into ascii (binary) and save
        fh.write(base64.decodebytes(img_data.encode('ascii')))
...
for s in ['hint-message', 'help-message']:
    ...
    # dataframe with help-fields / hint-fields only
    dfa = df_raw.loc[df_raw['odk_type'] == s, ['xml-parent', 'value']]
    drop_index = df_raw.loc[df_raw['odk_type'] == s, 'id']
    # in order to join dfa and df on index
    dfa.set_index('xml-parent', inplace=True)
    sa = s[:-8]+'::en'
    dfa.rename(columns={'value': sa}, inplace=True)
    df = df.join(dfa)  # this adds the help message column to df
    # remove 'help' rows from df (that data is now in the 'help' column)
...
df_new_arrow_sources.rename({'xml-parent': 'container_id',
                            'odk_type': 'odk_type_of_content'}, axis=1, inplace=True)

# add also the type of the container (page or hint-image)
df_new_arrow_sources = df_new_arrow_sources.merge(
    df[['odk_type']], how='left', left_on='container_id', right_index=True)
...
m = df_raw['xml-parent'].isin(container_ids) & ~df_raw['style'].str.contains(
    'image', na=False) & ~df_raw['odk_type'].isin(['hint-message', 'help-message'])
# used for dropping the labels from df after uploading info to container rows
label_ids = list(df_raw[m]['id'])
# all the label-children of containers
df_label = df_raw.loc[m, ['xml-parent', 'value', 'odk_type', 'name', 'id']]
...
df_label.loc[df_label.duplicated(
    subset=['name']), 'name'] = df_label['name'] + df_label['id']
...
df_hint_media_objects = df_new_arrow_sources.loc[df_new_arrow_sources['odk_type']
                                                 == 'container_hint_media']
df_arrows = df_arrows.merge(df_hint_media_objects,
...
# replace the source by the container-hint-media itself
df_arrows.loc[m, 'source'] = df_arrows.loc[m, 'container_id']
df_arrows.loc[m, 'source_type'] = df_arrows.loc[m, 'odk_type_of_content']
df_arrows.drop(columns=['container_id',
               'odk_type_of_content', 'container_type'], inplace=True)
...
# thing.
df_raw2 = df_raw.copy()
df_raw2.loc[df_raw2.duplicated(subset=['name'], keep='first') & ~df_raw2['name'].str.contains('opt_', na=False) & ~df_raw2['name'].str.contains(
    'stored_', na=False) & ~df_raw2['name'].str.contains('shortcut_', na=False), 'name'] = df_raw2['name']+df_raw2['id']
...
df_choices.rename(columns={'label': 'label::en',
                  'image': 'image::en'}, inplace=True)
...
df_choices.loc[df_choices['image::en'].str.contains(
    '.png', na=False), 'image::en'] = 'png-image' + df_choices['image::en']
df_choices.loc[df_choices['image::en'].str.contains(
    '.jpeg', na=False), 'image::en'] = 'jpeg-image' + df_choices['image::en']
...
df['color'] = df['style'].apply(lambda x: re.search(pattern, x).group(1) if re.search(
    pattern, x) and re.search(pattern, x).group(1) != 'none' else '#FFFFFF')
...
df_arrows = df_arrows.merge(
    df[['name', 'odk_type']], how='left', left_on='source', right_index=True)
# moving the type of the source into the column 'source_type'
df_arrows.loc[df_arrows['source_type'] == '',
              'source_type'] = df_arrows.loc[df_arrows['source_type'] == '', 'odk_type']
...
df_arrows = df_arrows.merge(df_choices[[
                            'list_name', 'name', 'odk_type']], how='left', left_on='source', right_index=True)
...
df_arrows.loc[df_arrows['source_type'] == 'select_one yesno',
              'expression'] = '${'+df_arrows['source_name'] + '}=' + '\'' + df_arrows.value + '\''
...
df_arrows.loc[df_arrows['source_type'] == 'text',
              'expression'] = '(${'+df_arrows['source_name'] + '}!=' + '\'\' or ${'+df_arrows['source_name'] + '}=' + '\'\')'

# expression for all the other select_one
df_arrows.loc[df_arrows['source_type'] == 'select_one',
              'expression'] = '${'+df_arrows['source_name'] + '}=' + '\'' + df_arrows['select_option'] + '\''

# expression for select_multiple
df_arrows.loc[df_arrows['source_type'] == 'select_multiple',
              'expression'] = 'selected(${'+df_arrows['source_name'] + '},\'' + df_arrows['select_option'] + '\')'

# expression for source being a calculate
df_arrows.loc[df_arrows['source_type'] == 'calculate',
              'expression'] = '${'+df_arrows['source_name'] + '}=1'
...
# mask for connectors that point to 'count' objects
m = df_arrows['target'].isin(df.loc[df['odk_type'] == 'count'].index)
df_arrows.loc[m,
              'expression'] = 'number(' + df_arrows.loc[m, 'expression'] + ')'

# add arrow weight to counter
m = df_arrows['value'].isin(['1', '2', '3']) & (
    df_arrows['target'].isin(df.loc[df['odk_type'] == 'count'].index))
df_arrows.loc[m, 'expression'] = df_arrows.loc[m, 'value'] + \
    ' * ' + df_arrows.loc[m, 'expression']
...
df_arrows.loc[m, 'source_name'] = df_arrows.loc[m,
                                                'source_name'].replace(r'^stored_', r'', regex=True)

# look up the odk_type that the rhombus is refering to
df_arrows = df_arrows.merge(
    df[['odk_type', 'name']], how='left', left_on='source_name', right_on='name')
...
df_arrows.rename(
    columns={'odk_type': 'rhombus_refer_to_odk_type'}, inplace=True)

# look up the value of the rhombus, it contains info about the logic
df_arrows = df_arrows.merge(
    df[['value']], how='left', left_on='source', right_index=True)
df_arrows.rename(columns={'value_x': 'value',
                 'value_y': 'value_of_rhombus'}, inplace=True)
...
m = ((df_arrows['source_type'] == 'rhombus') & (df_arrows['rhombus_refer_to_odk_type'].isin(
    ['integer', 'decimal']))) | (df_arrows['source_name'] == 'p_age')
df_arrows.loc[m, 'value_of_rhombus'] = df_arrows.loc[m, 'value_of_rhombus'].str.replace(
    r'^[^<=>]+', '', regex=True)  # only keep what comes after <,= or >
df_arrows.loc[m, 'value_of_rhombus'] = df_arrows.loc[m, 'value_of_rhombus'].str.replace(
    '?', '', regex=False)  # remove the '?' at the end
df_arrows.loc[m, 'expression'] = '${' + \
    df_arrows['source_name'] + '}' + df_arrows['value_of_rhombus']
df_arrows.loc[m & (df_arrows['value'] == 'No')] = df_arrows.loc[m & (
    df_arrows['value'] == 'No')].replace({'<=': '>', '>=': '<', '<': '>=', '>': '<='}, regex=True)

# when rhombus refers to a select_one yesno
m = (df_arrows['source_type'] == 'rhombus') & (
    df_arrows['rhombus_refer_to_odk_type'] == 'select_one yesno')
df_arrows.loc[m, 'expression'] = '${' + \
    df_arrows['source_name'] + '}=' + '\'' + df_arrows.value + '\''
...
m = (df_arrows['source_type'] == 'rhombus') & df_arrows['rhombus_refer_to_odk_type'].str.contains(
    'select_', na=False) & (df_arrows['rhombus_refer_to_odk_type'] != 'select_one yesno')
df_arrows.loc[m, 'value_of_rhombus'] = df_arrows.loc[m,
                                                     'value_of_rhombus'].str.extract(r'\[(.*?)\]', expand=False)
# merge again with df_choices to get the 'name' of the selected option (also needed for select_multiple!)
df_arrows = df_arrows.merge(df_choices[['list_name', 'name', 'label::en']],                 how='left', left_on=[
                            'source_name', 'value_of_rhombus'], right_on=['list_name', 'label::en'])
# when the outgoing arrow is YES (means that what is in RHOMBUS is TRUE)
df_arrows.loc[m & (df_arrows['value'] == 'Yes'),
              'expression'] = '${'+df_arrows['source_name'] + '}=' + '\'' + df_arrows['name'] + '\''
# when the outgoing arrow is NO (means that what is in RHOMBUS is FALSE)
df_arrows.loc[m & (df_arrows['value'] == 'No'),
              'expression'] = '${'+df_arrows['source_name'] + '}!=' + '\'' + df_arrows['name'] + '\''
...
# when the outgoing arrow is YES (means that what is in RHOMBUS is TRUE)
df_arrows.loc[m & (df_arrows['value'] == 'Yes'),
              'expression'] = 'selected(${'+df_arrows['source_name'] + '},\'' + df_arrows['name'] + '\')'
# when the outgoing arrow is NO (means that what is in RHOMBUS is FALSE)
df_arrows.loc[m & (df_arrows['value'] == 'No'),
              'expression'] = 'not(selected(${'+df_arrows['source_name'] + '},\'' + df_arrows['name'] + '\'))'


# when rhombus refers to calculate (AVOID THAT REFERENCE TO p_age GETS OVERWRITTEN!!! (SEE REFER TO INTEGER))
m = (df_arrows['source_type'] == 'rhombus') & (
    df_arrows['rhombus_refer_to_odk_type'] == 'calculate') & (df_arrows['source_name'] != 'p_age')
# when the outgoing arrow is YES (means that what is in RHOMBUS is TRUE)
df_arrows.loc[m & (df_arrows['value'] == 'Yes'),
              'expression'] = '${'+df_arrows['source_name'] + '}=1'
# when the outgoing arrow is NO (means that what is in RHOMBUS is False)
df_arrows.loc[m & (df_arrows['value'] == 'No'),
              'expression'] = '${'+df_arrows['source_name'] + '}=0'
...
m = (df_arrows['source_type'] == 'rhombus') & (
    df_arrows['rhombus_refer_to_odk_type'] == 'count')
df_arrows.loc[m, 'value_of_rhombus'] = df_arrows.loc[m, 'value_of_rhombus'].str.replace(
    r'^[^<=>]+', '', regex=True)  # only keep what comes after <,= or >
df_arrows.loc[m, 'value_of_rhombus'] = df_arrows.loc[m, 'value_of_rhombus'].str.replace(
    '?', '', regex=False)  # remove the '?' at the end
...
# mask for connectors that point to 'count' objects
m1 = df_arrows['target'].isin(df.loc[df['odk_type'] == 'count'].index)
...
for elem, group in gk:
    ...
    m2 = (df_arrows['source_name'] == counter_name) & (
        df_arrows['value'] == 'No')
    df_arrows.loc[m & m2, 'value_of_rhombus'] = df_arrows.loc[m & m2, 'value_of_rhombus'].replace(
        {'<=': '>', '>=': '<', '<': '>=', '>': '<=', '=': '!=', '!=': '='}, regex=True)
    df_arrows.loc[m & (df_arrows['source_name'] == counter_name),
                  'expression'] = full_expression + df_arrows['value_of_rhombus']

# In[33]:


# also drop the arrows that point to counters
df_arrows = df_arrows[df_arrows['target'].isin(
    df.loc[df['odk_type'] != 'count'].index)]

# drop no longer necessary columns
df_arrows.drop(columns=['value', 'value_of_rhombus', 'source_name',
               'rhombus_refer_to_odk_type', 'list_name', 'label::en', 'name'], inplace=True)
...
dfa = dfa.merge(df_raw[['id', 'xml-parent']], how='left',
                left_on='xml-parent', right_on='id', suffixes=('', '_y'))
# and if it is a container-hint-media, replace the 'xml-parent' of the select_option with the id of the container
container_ids = list(
    df_raw.loc[df_raw['odk_type'] == 'container_hint_media', 'id'])
...
# for connectors where the source is inside a container-hint-media, replace the source with the container itself
df_arrows = df_arrows.merge(
    df_new_arrow_sources, how='left', left_on='source', right_index=True)
df_arrows.fillna('', inplace=True)
df_arrows.rename(columns={'odk_type': 'container_type'}, inplace=True)
m = (df_arrows['container_type'] == 'container_hint_media')
df_arrows.loc[m, 'source'] = df_arrows.loc[m, 'container_id']
...
container_ids = df_arrows.loc[df_arrows['container_type']
                              == 'container_page', 'container_id'].unique()
...
page_starts_rhombus = df.loc[page_starts].loc[df['odk_type']
                                              == 'rhombus'].index
# get the page_objects where all objects in a single page are notes (needed for later)

# get page_start - container_id pairs
dfnew_connectors = df.loc[page_starts, [
    'xml-parent']].reset_index().rename(columns={'id': 'target', 'xml-parent': 'source'})

# add missing columns
dfnew_connectors = dfnew_connectors.reindex(
    columns=['source', 'target', 'source_type', 'expression', 'container_id', 'container_type'])
...
df_arrows = df_arrows.merge(
    df['odk_type'], how='left', left_on='target', right_index=True)
df_arrows.rename(columns={'odk_type': 'target_type'}, inplace=True)


# %% Build a graph
df_edges = df_raw.loc[df_raw['edge'] == '1']  # get all arrows from df_raw
df_edges = df_edges[(df_edges['source'] != '') & (
    df_edges['target'] != '')]  # remove some artefact objects

dag = nx.from_pandas_edgelist(
    df_edges, source='source', target='target', create_using=nx.DiGraph)  # build a graph
...
dag.remove_nodes_from(
    df_raw[df_raw['style'].str.contains('image', na=False)]['id'])
...
while nodecount0 < nodecount:
    ...
    # elements that are origins
    rootelements = [n for n, d in dag.in_degree() if d == 0]

    # get the parents of the rootelements (pages or select_xxx or container_hint_media)
    m = df_raw['odk_type'].str.contains('container_', na=False) | df_raw['odk_type'].isin([
        'select_one', 'select_multiple'])
    parent_ids = list(df_raw[m]['id'])  # ids of parents of root elements:
    df_roots = df_raw.loc[df_raw['id'].isin(
        rootelements) & df_raw['xml-parent'].isin(parent_ids)]
...
df_shortcuts.replace({'name': r'^shortcut_'}, {
                     'name': ''}, regex=True, inplace=True)
...
node_to_shortcut = dict(zip([x[1] for x in node_to_shortcut], [
                        x[0] for x in node_to_shortcut]))

# replacing name of exit node by its id
df_shortcuts['name'].replace(exit_nodes_dict, inplace=True)
# replacing id of shortcut by id of parent node
df_shortcuts['id'].replace(node_to_shortcut, inplace=True)
...
diagnosis_id_hierarchy = gt.get_diagnosis_sorting_id_jupyter(
    df_raw, p.diagnosis_order)  # make diagnosis id hierarchy list
...
rootelements = [n for n, d in dag.in_degree(
) if d == 0 and dag.nodes[n]['content'] != '']
...
non_dataloader_rootelement = [n for n in rootelements if dataloader_id not in list(
    dag.successors(n)) and dag.nodes[n]['content'] != ''][0]
...
dataloader_rootelement = [n for n in rootelements if dataloader_id in list(
    dag.successors(n)) and dag.nodes[n]['content'] != ''][0]
...
# print(df.drop_duplicates().index.tolist())
# df is now sorted, but there are many rows from df_raw, that no longer exist in df
df = df.reindex(I_new)
...
df_pageObjects = df.loc[df['xml-parent'].isin(
    df.loc[df['odk_type'] == 'container_page'].index)]
...
df_notes_out_pages = df_arrows.loc[df_arrows['source'].isin(df_pageObjects.index) & ~df_arrows['target'].isin(
    df_pageObjects.index) & (df_arrows['source_type'] == 'note')]

# among those get those notes that belong to 'pure_note_pages' - these are the notes you are looking for
df_notes_outof_pure_notes_pages = df_notes_out_pages.loc[df_notes_out_pages['container_id'].isin(
    pure_note_pages)]
df_notes_outof_pure_notes_pages = df_notes_outof_pure_notes_pages[[
    'source', 'container_id']]
...
for elem in df.index:
    ...
    df_sources = df_arrows.loc[df_arrows['target'] ==
                               elem, ['source', 'source_type', 'expression']]
    ...
    df_sources = df_sources.merge(
        df[['relevant', 'xml-parent']], how='left', left_on='source', right_index=True)
    ...
    df_sources = df_sources.merge(
        df[['relevant']], how='left', left_on='xml-parent', right_index=True, suffixes=('', '_page'))
    m = df_sources['source_type'].isin(['rhombus']) & (df_sources['relevant'] == '') & (
        df_sources['relevant_page'] != '') & df_sources['xml-parent'].isin(container_ids)
    df_sources.loc[m, 'expression'] = df_sources.loc[m, 'expression'] + \
        ' and (' + df_sources.loc[m, 'relevant_page'] + ')'

    # when the source is a rhombus and it's relevant is NOT empty, you have to combine both with AND
    m = df_sources['source_type'].isin(
        ['rhombus']) & (df_sources['relevant'] != '')
    df_sources.loc[m, 'expression'] = df_sources.loc[m,
                                                     'expression'] + ' and (' + df_sources.loc[m, 'relevant'] + ')'
    ...
    # its expression
    # Mask to get pure note elements
    m = df_sources['source'].isin(df_notes_outof_pure_notes_pages.index)
    ...
    df_sources.loc[df_sources['expression'] != '',
                   'expression'] = '(' + df_sources.loc[df_sources['expression'] != '', 'expression'] + ')'

    if df.loc[elem, 'odk_type'] != 'count':
        # to reduce repetitions in expression
        expressions = set(filter(None, df_sources['expression']))
        ...
        df.loc[elem,
               'relevant'] = ' + '.join(filter(None, df_sources['expression']))
        if '( and (' in df.loc[elem, 'relevant'] or '( or (' in df.loc[elem, 'relevant']:
            print(elem, 'error!')
            print(df.loc[elem, 'relevant'])
...
df = df.merge(df[['id', 'new_index']], how='left',
              left_on='xml-parent', right_on='id', suffixes=('', '_p'))
...
for name, frame in g:  # for each page
    ...
    for i in frame.index:  # for each element in that page
        if df.loc[i, 'xml-parent'] in pageids:  # if we are in a real page and not in root
            df.loc[i, 'new_index'] = df.loc[i, 'new_index'] + \
                k  # add to the new index a small step
            k += 0.001
...
df.reset_index(drop=False, inplace=True)
index_endgrouprows = df.loc[~df.duplicated(
    subset='xml-parent', keep='last') & df['xml-parent'].isin(pageids)].index+0.1
...
df = df[['odk_type', 'name', 'value', 'help::en', 'hint::en', 'appearance', 'relevant', 'constraint',
         'constraint_message', 'required', 'required message::en', 'calculation', 'repeat_count', 'image::en', 'color']]
df.rename(columns={'odk_type': 'type', 'value': 'label::en', 'relevant': 'relevance',
          'constraint_message': 'constraint message::en'}, inplace=True)
...
df.loc[df['type'] == 'calculate',
       'calculation'] = df.loc[df['type'] == 'calculate', 'relevance']
# add 'number() to fit with odk '
df.loc[df['type'] == 'calculate',
       'calculation'] = 'number(' + df.loc[df['type'] == 'calculate', 'calculation'] + ')'
...
tt_input_options = df.loc[(df['type'] == 'calculate') & df['name'].str.contains(
    'load_', na=False), ['type', 'name', 'label::en']]


# tt_input_options are 'contextual parameters'
tt_input_options.rename(columns={'type': 'list_name'}, inplace=True)
tt_input_options['list_name'] = 'data_load'
# concat the new options to df_choices
df_choices = pd.concat([df_choices, tt_input_options])

# make the first question for data load
# data_load = ['select_multiple calculate','data_load','Define adaptable parameters','','','','','','','','','','','']
#
data_load = ['select_multiple data_load', 'data_load',
...
for image in p.images_to_import:
    ...
    import_note = pd.DataFrame({'type': 'note', 'name': 'import_'+str(identifier),
                               'relevance': 'false()', 'image::en': image, 'image::fr': image}, index=[int(identifier)])
    df = pd.concat([import_note, df], ignore_index=False)
# populate the load_ calculate fields
df.loc[(df['type'] == 'calculate') & df['name'].str.contains('load_', na=False),
       'calculation'] = 'number(selected(${data_load}, \'' + df.loc[df['type'] == 'calculate', 'name'] + '\'))'
...
df.loc[df['type'].str.contains('select_multiple', na=False),
       'constraint'] = '.=\'opt_none\' or not(selected(.,\'opt_none\'))'
df.loc[df['type'].str.contains('select_multiple', na=False),
       'constraint message::en'] = '**None** cannot be selected together with symptoms.'
...
if p.form_id == 'yi':
    ...
    df.loc[df['name'] == 'p_age',
           'calculation'] = 'coalesce(${p_age_days},${p_age_weeks}*7)'
...
df.fillna('', inplace=True)
I = df.loc[df['name'].isin(diagnoses_dict.values()) & ~df['name'].isin(
    p.hide_diagnoses) & ~df['name'].str.startswith('as_')].index
counter = 0
for i in I:
...
df.loc[m, 'label::en'] = '<span style="color: rgb(163, 92, 56);">' + \
    df.loc[m, 'label::en'] + '</span>'
...
df.loc[~df['type'].isin(['note', 'calculate', 'begin group', 'end group',
                        'text', 'acknowledge', '']) & (df['required'] == ''), 'required'] = '1'
...
if p.form_id == 'ped':
    df.loc[df['name'] == 'p_age',
           'calculation'] = "if(${p_age_select}='2',2,if(${p_age_select}='4',4,if(${p_age_select}='6',6,if(${p_age_select}='12',12,if(${p_age_select}='24',24,if(${p_age_select}='36',36,48))))))"
...
def make_summary(df, df_choices, diagnose_id_hierarchy, summaryfile, df_diagnose_list, diagnoses_to_hide):
    ...
    for i, row in df_diagnose_list.iterrows():
        if row['id'].startswith('as') or row['id'] in (diagnoses_to_hide):
            continue
        ...
        if not pd.isnull(row['postcoordination']):
            ...
            for postcord in postcoordinations:
                ...
                counter = 1 if postcord not in postcord_counter else postcord_counter.get(
                    postcord)+1
                temp = pd.DataFrame({
                    'calculation': '',
                ...
                try:
                    ...
                    postcord_counter[postcord] = 1
                df_diagnose = pd.concat([df_diagnose, temp])
    if p.form_id == 'msf_sti':
        asymptomatic_no_tests = pd.DataFrame({
            'calculation': '',
            'relevance': 'not(selected(${mc_age}, \'0-42\')) and number(${as_all_negative_tests})=1 and number(${as_asymptomatic})=1 and number(${p_high_risk})=0',
            'appearance': 'center',
            'type': 'note',
            'label::en': '<p> Asymptomatic, tests negative. Provide counselling. </p>',
            'name': 'asymptomatic_negative_tests'}, index=[0])
        high_risk_no_tests = pd.DataFrame({
            'calculation': '',
            'relevance': 'not(selected(${mc_age}, \'0-42\')) and number(${p_high_risk})=1 and number(${as_all_negative_tests})=1 and number(${as_asymptomatic})=0',
            'appearance': 'center',
            'type': 'note',
            'label::en': '<p> High risk population but tests negative. Provide counselling. </p>',
            'name': 'highrisk_negative_tests'}, index=[0])
        high_risk_asymptomatic_no_tests = pd.DataFrame({
            'calculation': '',
            'relevance': 'not(selected(${mc_age}, \'0-42\')) and number(${p_high_risk})=1 and number(${as_all_negative_tests})=1 and number(${as_asymptomatic})=1',
            'appearance': 'center',
            'type': 'note',
            'label::en': '<p> Asymptomatic high risk population all tests negative. Provide counselling. </p>',
            'name': 'highrisk_asympt_negative_tests'}, index=[0])
        df_diagnose = pd.concat([df_diagnose, asymptomatic_no_tests])
        df_diagnose = pd.concat([df_diagnose, high_risk_no_tests])
        df_diagnose = pd.concat([df_diagnose, high_risk_asymptomatic_no_tests])
    ...
    df_medical_tests = df.loc[df['name'].isin(diagnose_id_hierarchy) & (
        df['name'].str.startswith('as'))].copy()
    df_tests_list = df_diagnose_list.loc[df_diagnose_list['id'].str.startswith(
        'as_', na=False) & df_diagnose_list['id'].str.endswith('test', na=False)]

    df_medical_tests['calculation'] = ''
    df_tests_list = df_tests_list.rename(
        columns={"name": "label::en", "id": "name", "relevant": "relevance"})
    ...
    df_medical_tests['label::en'] = '<p>' + \
        df_medical_tests['label::en'] + '</p>'
    df_medical_tests['name'] = df_medical_tests['name'].replace(
        {'as_': 'label_'}, regex=True)
    ...
    df_summary.loc[df_summary['name'] ==
                   'g_positive_tests_summary', 'relevance'] = ds_relevance
...
df_summary = make_summary(df, df_choices, diagnosis_id_hierarchy,
                          p.summaryfile, df_diagnose, p.hide_diagnoses)
# store df_summary
