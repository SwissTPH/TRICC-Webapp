...
# logic table for the relevance field of caretaker advice elements
cafile = p.folder+'ca.xlsx'
...
# In[10]:
df_survey_dx = pd.read_excel(
    p.folder+p.form_id+'_dx.xlsx', sheet_name='survey')
df_survey_tt = pd.read_excel(p.output_xls, sheet_name='survey')
df_choices_dx = pd.read_excel(
    p.folder+p.form_id+'_dx.xlsx', sheet_name='choices')
...
df_tt.drop(df_tt.loc[df_tt['name'].isin(
    df_diagnoses['id'])].index, inplace=True)
# all rows that also appear in the headerfile
df_header = pd.read_excel(p.headerfile)
df_tt.drop(df_tt.loc[df_tt['name'].isin(
    df_header['name'])].index, inplace=True)
...
df_tt.drop(df_tt.loc[(df_tt['type'] == 'note') & (
    df_tt['name'] == 'data_loader')].index, inplace=True)
# the rows with 'select_multiple select_diagnosis'
df_tt.drop(df_tt.loc[(df_tt['type'] ==
           'select_multiple select_diagnosis')].index, inplace=True)
# the rows with 'select_multiple select_dataload'
df_tt.drop(
    df_tt.loc[(df_tt['type'] == 'select_multiple data_load')].index, inplace=True)


# %% wrap treatment in a group
dfa = pd.DataFrame(columns=df_tt.columns)
tt_relevance = df_dx.loc[df_dx['label::en'] == 'TREATMENT',
                         'relevance'].values[0]  # get relevance field for TT group
# field-list taken out to stop have TT in one page MPEA
dfa.loc[0, ['type', 'name', 'label::en', 'appearance', 'relevance']] = [
    'begin group', 'g_tt', 'Treatment and Management', 'field-list', tt_relevance]
...
drops = df.loc[df.duplicated(subset='name', keep='first') & (df['name'] != '') & (
    df['name'] != 'data_load') & ~df['type'].str.contains('group')].index
...
df.drop(df.loc[df.duplicated(subset='name', keep=False) & (
    df['name'] != '') & (df['name'] != 'data_load')].index, inplace=True)
...
# version_idx = df.loc[df['name']=='data_load'].index[0]-0.5
# version_algo = pd.DataFrame({'type':'note', 'name':'version', 'label::en': 'Algorithm version: '+str(p.version)}, index=[version_idx])
...
if p.context_params_from_facility == True:
    ...
    # select_multiple row
    df.drop(df.loc[(df['name'] == 'data_load') & df['type'].str.contains(
        'select_', na=False)].index, inplace=True)  # drop manual data_load selector
...
droprows = df_tt[['list_name', 'name']].reset_index().merge(
    df_dx[['list_name', 'name']], how='inner', on=['list_name', 'name']).set_index('index').index
...
df_tt.drop(df_tt.loc[(df_tt['list_name'] == 'data_load') & ~
           df_tt['name'].str.contains('load_', na=False)].index, inplace=True)
...
settings = {'form_title': p.form_title, 'form_id': form_id,
...
m = df_survey.columns.str.contains(
    'label') | df_survey.columns.str.contains('help')
...
df_survey.loc[m, 'label::en'] = df_survey.loc[m,
                                              'label::en'].apply(clh.make_green_background)
...
if p.testing == True:
    m = (df_survey['name'] != '') & ~df_survey['type'].str.contains(
        'group', na=False)
    df_survey.loc[m, 'label::en'] = '---' + df_survey.loc[m,
                                                          'name'] + '---\n' + df_survey.loc[m, 'label::en']
    df_survey.loc[m, 'label::fr'] = '---' + df_survey.loc[m,
                                                          'name'] + '---\n' + df_survey.loc[m, 'label::fr']
    timestamprow_i = df_survey.loc[df_survey['name']
                                   == 'data_load'].index[0]-0.5
    timestamprow_i = int(timestamprow_i)
    time = datetime.now()
    timestamprow = pd.DataFrame({'type': 'note', 'name': 'timestamp',
                                'label::en': 'Timestamp of build: '+str(time)}, index=[timestamprow_i])
...
if form_id == 'ped':
    ...
    df_survey.loc[df_survey['name'] == 'select_symptoms_other',
                  'choice_filter'] = 'Min_age_months<=${p_age}'
    df_survey.loc[df_survey['name'] == 'major_symptoms',
                  'choice_filter'] = 'Max_p_temp>=${p_temp}'
    df_choices['Min_age_months'] = ''
    df_choices['Max_p_temp'] = ''
    df_choices.loc[(df_choices['list_name'] ==
                    'select_symptoms_other'), 'Min_age_months'] = '2'
    df_choices.loc[(df_choices['list_name'] ==
                    'major_symptoms'), 'Max_p_temp'] = '45'
    df_choices.loc[(df_choices['list_name'] == 'select_symptoms_other') &
                   df_choices['name'].isin(['opt_9', 'opt_10']), 'Min_age_months'] = '24'
    df_choices.loc[(df_choices['list_name'] == 'select_symptoms_other') &
                   df_choices['name'].isin(['opt_9', 'opt_10']), 'Min_age_months'] = '24'
    df_choices.loc[(df_choices['list_name'] == 'major_symptoms')
                   & df_choices['name'].isin(['opt_1']), 'Max_p_temp'] = '37.5'
...
if p.interrupt_flow:
    ...
    # drop the begin inputs group row, because it is created by the make_breakpoints function
    df_inputgroup_a.drop(0, inplace=True)
    # drop last row of the cht head file because the other xls has the same variable
    df_inputgroup = df_inputgroup.iloc[:-1]
    ...
    pause_old_relevance = df_survey.loc[df_survey['name']
                                        == 'label_form_pause', 'relevance']
...
if p.interrupt_flow:
    for i in breaks:
        ...
        d[newsettings_frame]['form_id'] = d[newsettings_frame]['form_id'] + \
            '_' + breakname

        # where group inputs end, paste all the stuff from ped_
        endinputsrow = dfa.loc[(dfa['type'] == 'end group') & (
            dfa['name'] == 'inputs')].index[0]
        df_inputgroup_a.index = df_inputgroup_a.index / \
            (len(df_inputgroup_a)+1)-1+endinputsrow
        # shift the elements that do not belong to group input out of group input
        index_a = df_inputgroup_a.loc[(df_inputgroup_a['type'] == 'end group') & (
            df_inputgroup_a['name'] == 'inputs')].index[0]
        ...
        duplicateendgrouprow = dfa.loc[(dfa['type'] == 'end group') & (
            dfa['name'] == 'inputs')].index[1]
...
if p.interrupt_flow:
    ...
    print(m)
    df_survey.loc[m, 'relevance'] = '( ' + df_survey.loc[m, 'relevance'] + \
        ') and (${ask_pause}=\'continue_consultation\' or ${ask_pause}=\'\')'

    # write back the old relevance of the label_pause_form
    df_survey.loc[df_survey['name'] == 'label_form_pause',
                  'relevance'] = pause_old_relevance

# %% for deceased cases in ped the ca is showing, find out later why.... no, don't, I don't want to know that.
if form_id == 'ped':
    df_survey.loc[df_survey['name'] == 'g_tt',
                  'relevance'] = '(' + df_survey.loc[df_survey['name'] == 'g_tt', 'relevance'] + ') and ${d_deceased}=0'

# %% making the add missing diagnose field mandatory
df_survey.loc[df_survey['name'] ==
              'text_missing_diagnose_add', 'required'] = '1'
...
os.makedirs(p.folder + 'output/',  exist_ok=True)
